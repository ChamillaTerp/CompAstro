...
class MUSCL(HD):
    ...
    def __init__(self, u, C=0.2, Solver=HLL, Slope=MonCen, Poisson=FFT):
        ...
        u.dPhi = np.zeros((3,)+n)     # gradient of potential
    ...
    def Prim(self,u):
        """ Compute 5 primitive variables D,P,v -- all centered """
        D = u.D
        v = u.velocity()
        P = u.pressure()
        u.prim[:,:,:,:] = np.array([D,P,v[0],v[1],v[2]])
    ...
    def Predict(self,u):
        ...
        D  = u.prim[0];  P  = u.prim[1];  v  = u.prim[2:5]
        dD = u.slope[0]; dP = u.slope[1]; dv = u.slope[2:5]
        dPhi = u.dPhi
        ...
        # dP/dt = -v*grad(P) - gamma*P*div(v) + (gamma-1) * Qrad
        u.predict[1] = P + 0.5*u.dtds* ( u.ds*(u.gamma-1)*u.rt.qtot
                       - np.sum(v*dP,axis=0) - u.gamma*P*div_v)
        if u.predict[1].min() < 0.:
            print('negative predicted pressure. Should not happen. Terms where:')
            print('P    :', P.min(), P.max())
            print('Q    :', (0.5*u.dtds*u.ds*(u.gamma-1)*u.rt.qtot).min(), (u.dt*(u.gamma-1)*u.rt.qtot).max())
            print('v*dP :', (-0.5*u.dtds*np.sum(v*dP,axis=0)).min(), (-0.5*u.dtds*np.sum(v*dP,axis=0)).max())
            print('gPdv :', (-0.5*u.dtds*u.gamma*P*div_v).min(), (-0.5*u.dtds*u.gamma*P*div_v).max())
            print('Ppred:', u.predict[1].min(), u.predict[1].max())

        # dv_i/dt = - v.grad(v_i) - 1/rho grad(P) - grad(Phi)
        for i in range(3):
            u.predict[2+i] = v[i] + 0.5*u.dtds* ( \
                             - v[0]*dv[i,0] - v[1]*dv[i,1] - v[2]*dv[i,2] - dP[i]/D - dPhi[i] )
    ...
    def Step(self,u,BCs=None,C=0.2,tend=None,dtmax=None):
        ...
        if not tend==None:
            ...
            if u.t+u.dt > tend:            # adjust dt if we will pass tend
                u.dt = tend - u.t
        u.dtds = u.dt / u.ds
        self.Prim(u)                       # Primitive vars       shape (nv,n,n,n)
